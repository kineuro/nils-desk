...
describe("the planes' level", () => {
  it("takes the finest level whose volume fits the budget", () => {
    // 220 x 1024 x 1024 at 16 bits is 461 MB: level 0 fits 512 MB
    expect(volumeLevel(manifest([220, 1024, 1024]))).toEqual({ level: 0, stride: 1, dims: [1024, 1024, 220], bytes: 220 * 1024 * 1024 * 2 });
    expect(volumeLevel(manifest([220, 1024, 1024]), 256 * 1024 * 1024)?.level).toBe(1);
  });
  it("takes every second plane of a stack deeper than the card's 3D texture, and a coarser level for the rest", () => {
    ...
    // a card that takes 4096 deep holds every plane at the same level
    expect(volumeLevel(manifest([2500, 1024, 1024]), VOLUME_BUDGET, 4096)).toMatchObject({ level: 2, stride: 1 });
  });
