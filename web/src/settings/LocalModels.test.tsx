...
// A model Kvasir downloads, as its card draws (record 25): a download under
// way with its bar, how far it is and a pause; a failed one with its error and
// a resume; a downloaded one with the commands a model server runs it with,
// each to copy, folded under the card with the line that says Kvasir runs no
// model. Where the install runs llama.cpp for Kvasir (record 24): Start for a
// model Kvasir can start, Stop while it loads or serves, whether Kvasir
// admitted it and the stations it answers, why it did not start with what
// llama.cpp logged, and the commands kept for the rest. And the line under the
// models: the machine and llama.cpp, where downloads go, and the token.
...
describe("a local model's card", () => {
  it("draws a download under way with its bar, how far it is, its files, a pause and a removal", () => {
    ...
    expect(html).toContain("This machine · from the Hugging Face Hub");
    expect(html).toContain("downloading");
    expect(html).toContain('role="progressbar"');
    expect(html).toContain('aria-valuenow="25"');
    expect(html).toContain("width:25%");
    expect(html).toContain("4 GiB of 16 GiB, 25%");
    expect(html).toContain('<details class="card-files"><summary>Its files</summary>');
  ...
  it("folds a downloaded model's commands under their labels, each to copy, and says Kvasir runs none", () => {
    ...
    expect(html).toContain("This machine · for a model server of yours");
...
describe("a local model's card where the install runs llama.cpp for Kvasir", () => {
  ...
  const drawn = (m: LocalModel, admission: Admission | null = null, answers: string | null = null) =>
  ...
  it("offers Start for a model Kvasir can start, and folds its commands under Or run it yourself", () => {
    ...
    expect(html).toContain("This machine · starts on llama.cpp");
  ...
  it("names a serving model as llama.cpp serves it, with whether Kvasir admitted it, the stations it answers, and Stop", () => {
    const html = drawn(model({ ...done, startable: true, run: run() }), { tone: "blocked", words: "refused on 15 Sept", detail: "failed tool calls" }, "answers ask-help and identity-check");
    expect(html).toContain('title="owner/name">name-q4-k-m</span>');
    expect(html).toContain("This machine · llama.cpp · 32,768 tokens");
    expect(html).toContain("serving</span>");
    expect(html).toContain("refused on 15 Sept</span>");
    expect(html).toContain("failed tool calls");
    expect(html).toContain("answers ask-help and identity-check");
    expect(html).toContain(">Stop</button>");
    expect(html).not.toContain(">Start</button>");
    const admitted = drawn(model({ ...done, startable: true, run: run() }), { tone: "ok", words: "admitted 14 Sept", detail: null });
    expect(admitted).toContain('<span class="meta">admitted 14 Sept</span>');
  ...
  it("offers Stop while a model loads", () => {
    const html = drawn(model({ ...done, startable: true, run: run({ state: "starting", context: null, slots: null }) }), null, "answers ask-help");
  ...
  it("says why a model did not start, with what llama.cpp logged behind a disclosure, and offers Start again", () => {
    ...
    const html = drawn(model({ ...done, startable: true, run: run({ state: "failed", error: "llama.cpp ended with exit code 1", log, context: null, slots: null }) }), null, "answers ask-help");
    ...
    expect(html).not.toContain("answers ask-help");
