...
// session board's tiles instead (review/SessionBoard.tsx).
...
export function StackView({ stack, level = null, view = "stack" }: { stack: number; level?: number | null; view?: "stack" | "planes" }) {
  return (
    <div className="stack-view">
      <Suspense fallback={<Wait phase="loading the viewer" since={Date.now()} size="panel" />}>
        <Viewer key={stack} stack={stack} level={level} view={view} />
      </Suspense>
    </div>
  );
}
